...
public class MauSacView extends javax.swing.JFrame {
    ...
    private MauSacDAO hangservice = new MauSacDAO();
    ...
    MauSac getForm() {
        ...
        MauSac hang = new MauSac(tenHang);
        return hang;
    }
        ...
        if (row >= 0) {
            ...
            if (confirm) {
                ...
                if (rowHienTai < list.size()) {
                    MauSac hang = getForm();
                    hang.setId(hangservice.getMauSac().get(rowHienTai).getId());
                    hangservice.UpdateMauSac(hang);
                    list = hangservice.getMauSac();
                    fillTable();
                    clear();
    ...
    private void tblHangMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_tblHangMouseClicked
        ...
        if (row >= 0) {
            int rowHienTai = (soTrang - 1) * soDong + row;
            if (rowHienTai < list.size()) {
                MauSac hang = list.get(rowHienTai);
                showTable(hang);
            }
        }
    ...
    private void btnSearchActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnSearchActionPerformed
        ...
        MauSac hang = hangservice.SearchMauSac(search);
        if (search == null) {
    ...
    private void btnNextPageActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnNextPageActionPerformed
        int maxPage = (int) Math.ceil((double) hangservice.getMauSac().size() / soDong);
        if (soTrang < maxPage) {
            soTrang++;
            fillTable();
        }
