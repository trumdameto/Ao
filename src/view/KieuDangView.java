...
public class KieuDangView extends javax.swing.JFrame {
    ...
    private KieuDangDAO hangservice = new KieuDangDAO();
    ...
    void showTable(KieuDang hang) {
        txtHang.setText(hang.getName());
    }
    ...
    KieuDang getForm() {
        ...
        if (tenHang.isEmpty()) {
            JOptionPane.showMessageDialog(this, "Chưa nhập tên kiểu dáng kìa");
            return null;
        }
        KieuDang hang = new KieuDang(tenHang);
        return hang;
    }
    ...
    private void btnSuaActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnSuaActionPerformed
        ...
        if (row < 0) {
            JOptionPane.showMessageDialog(this, "Chọn kiểu dáng cần sửa!");
            return;
        }
        if (row >= 0) {
           boolean confirm = JOPane.showConfirmDialog(this, "Are you sure?");
            if (confirm) {
                ...
                KieuDang hang = getForm();
                hang.setId(hangservice.getKieuDang().get(rowHienTai).getId());
                hangservice.UpdateKieuDang(hang);
                list = hangservice.getKieuDang();
                fillTable();
                clear();
    ...
    private void tblHangMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_tblHangMouseClicked
        int row = tblHang.getSelectedRow();
                    if (row >= 0) {
                        int rowHienTai = (soTrang - 1) * soDong + row;
                        if (rowHienTai < list.size()) {
                            KieuDang hang = list.get(rowHienTai);
                            showTable(hang);
                        }
                    }
    ...
    private void btnXoaActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnXoaActionPerformed
        ...
        if (row >= 0) {
            ...
            if (confirm) {
                ...
                hangservice.DeleteKieuDang(name);
                list = hangservice.getKieuDang();
                fillTable();
                clear();
    ...
    private void btnSearchActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnSearchActionPerformed
        ...
        KieuDang hang = hangservice.SearchKieuDang(search);
        if (search == null) {
            JOptionPane.showMessageDialog(this, "Nhập tên kiểu dáng cần tìm !");
        } else {
            boolean confirm = JOPane.showConfirmDialog(this, "Are you sure?");
            if (confirm) {
                if (search.equals(hang.getName())) {
                    showTable(hang);
                    JOptionPane.showMessageDialog(this, "Tìm thấy kiểu dáng tên: " + hang.getName());
                }
            }
    ...
    private void btnNextPageActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnNextPageActionPerformed
        int maxPage = (int) Math.ceil((double) hangservice.getKieuDang().size() / soDong);
        if (soTrang < maxPage) {
            soTrang++;
            fillTable();
        }
