...
public class DanhMucView extends javax.swing.JFrame {
    ...
    private DanhMucDAO hangservice = new DanhMucDAO();
    ...
    DanhMuc getForm() {
        ...
        DanhMuc hang = new DanhMuc(tenHang);
        return hang;
    }
        ...
        if (row >= 0) {
            ...
            if (confirm) {
                ...
                if (rowHienTai < list.size()) {
                    DanhMuc hang = getForm();
                    hang.setId(hangservice.getDanhMuc().get(rowHienTai).getId());
                    hangservice.UpdateDanhMuc(hang);
                    list = hangservice.getDanhMuc();
                    fillTable();
                    clear();
    ...
    private void tblHangMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_tblHangMouseClicked
        ...
        if (row >= 0) {
            int rowHienTai = (soTrang - 1) * soDong + row;
            if (rowHienTai < list.size()) {
                DanhMuc hang = list.get(rowHienTai);
                showTable(hang);
            }
        }
    ...
    private void btnXoaActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnXoaActionPerformed
        ...
        if (row >= 0) {
            ...
            if (confirm) {
                ...
                hangservice.DeleteDanhMuc(name);
                list = hangservice.getDanhMuc();
                fillTable();
                clear();
    ...
    private void btnSearchActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnSearchActionPerformed
        ...
        DanhMuc hang = hangservice.SearchDanhMuc(search);
        if (search == null) {
            JOptionPane.showMessageDialog(this, "Nhập tên hãng cần tìm !");
        } else {
            boolean confirm = JOPane.showConfirmDialog(this, "Are you sure?");
            if (confirm) {
                if (search.equals(hang.getName())) {
                    showTable(hang);
                    JOptionPane.showMessageDialog(this, "Tìm thấy hãng tên: " + hang.getName());
                }
            }
    ...
    private void btnNextPageActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnNextPageActionPerformed
        int maxPage = (int) Math.ceil((double) hangservice.getDanhMuc().size() / soDong);
        if (soTrang < maxPage) {
            soTrang++;
            fillTable();
        }
    ...
    public static void main(String args[]) {
        ...
        try {
            ...
            java.util.logging.Logger.getLogger(DanhMucView.class
